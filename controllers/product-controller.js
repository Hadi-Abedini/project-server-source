...
const resizeProductImages = async (productId, files) => {
	...
	const resizedImages = await Promise.all(
		images.map(async (image, index) => {
			const imageFilename = `products-${productId}-${Date.now()}-${index + 1
				}.jpeg`;
...
const getAllProducts = async (req, res, next) => {
	const productsModel = new ApiFeatures(Product.find({}).populate({
			path: 'category subcategory',
			select: 'name'
		}).populate("rating"), req.query)
		.limitFields()
		.paginate()
		.filter()
		.sort();
...
const addProduct = async (req, res, next) => {
	...
	if (isProductExists) {
		return next(
			new AppError(
				409,
				'نام کالا تکراری است، نام جدیدی انتخاب کنید'
			)
		);
	}
